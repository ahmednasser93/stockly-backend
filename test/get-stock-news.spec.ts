...
describe("getStockNews handler", () => {
  ...
  it("requires a symbol", async () => {
    const response = await getStockNews(createUrl(), createEnv());
  ...
  it("returns cached data without calling the upstream API", async () => {
    ...
    const response = await getStockNews(createUrl({ symbol: "aapl" }), env);
  ...
  it("returns empty array when FMP API returns empty array", async () => {
    ...
    const response = await getStockNews(createUrl({ symbol: "XYZ" }), env);
