...
export async function getHistorical(url: URL, env: Env, ctx?: ExecutionContext): Promise<Response> {
  ...
  try {
    ...
    if (data.length === 0 || hasMissingOHLC) {
        ...
        console.log(`[get-historical] ⚠️  Found ${recordsWithMissingOHLC} records with missing OHLC data out of ${data.length} total, re-fetching from FMP API to populate OHLC fields...`);
      }
      if (data.length === 0) {
        console.log(`[get-historical] No historical data in database for ${normalizedSymbol} (${formatDate(fromDate!)} to ${formatDate(toDate!)}), fetching from FMP API...`);
      }
      ...
      try {
        ...
        console.log(`[get-historical] After FMP fetch, query result for ${normalizedSymbol} (${formatDate(fromDate!)} to ${formatDate(toDate!)}): ${data.length} records found`);
        
        if (data.length === 0) {
          console.warn(`[get-historical] Still no data after fetching from FMP API for ${normalizedSymbol}. Possible reasons:
            - FMP API returned empty data
            - FMP API returned data outside the requested date range
            - Date range ${formatDate(fromDate!)} to ${formatDate(toDate!)} is in the future or has no trading days
